...
/// Stop a wedged compiler before it can hold the single production build slot
/// forever. The browser timeout is deliberately longer so the server can
/// return an actionable error while the request is still open.
const BUILD_TIMEOUT_SECONDS: u64 = 240;
...
pub fn build(
    ...
    // Every request uses the dependency artifacts baked into the image. Production limits builds
    // to one at a time so the shared `solpg` output cannot race between learner workspaces.
    let target_dir = std::env::current_dir()?.join(PROGRAMS_DIR).join("target");
    let output = Command::new("timeout")
        ...
        .arg("--signal=TERM")
        .arg("--kill-after=5s")
        .arg(format!("{BUILD_TIMEOUT_SECONDS}s"))
        .arg("cargo-build-sbf")
        .arg("--manifest-path")
        .arg(manifest_path)
        .arg("--sbf-out-dir")
    ...
    if matches!(output.status.code(), Some(124 | 137)) {
        return Err(anyhow!(
            "Program build exceeded {BUILD_TIMEOUT_SECONDS} seconds and was stopped. Try again; if it keeps happening, ask Program Pal to explain the build output."
        ));
    }
