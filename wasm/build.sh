...
build() {
    ...
    if [ "$package" = "rust-analyzer" ]; then
        ...
        # Modern webpack requires fully-specified ESM imports. The Rayon worker
        # snippet points at the package directory, so make the generated package
        # entrypoint explicit.
        for snippet in pkg/snippets/*/src/workerHelpers.js; do
            if [ ! -f "$snippet" ]; then
                continue
            fi

            if [[ "$(uname)" == "Darwin" ]]; then
                sed -i '' "s#import('../../..')#import('../../../${package_name}.js')#" "$snippet"
            else
                sed -i "s#import('../../..')#import('../../../${package_name}.js')#" "$snippet"
            fi
        done
