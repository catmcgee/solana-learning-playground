...
async function checkProgram() {
  ...
  if (!PgProgramInfo.onChain) {
    throw new Error(
      `Could not fetch onchain data. Try using a different RPC provider with '${PgTerminal.bold(
  ...
  if (userBalance < requiredBalanceWithoutFees) {
    ...
    const msg = `${
      ...
      programExists
        ...
        : "This is the cost to store the program onchain. You can reclaim the funds by closing the program later."
    }`;
...
/** Get the current program's onchain program data account length */
