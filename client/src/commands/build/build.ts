...
export const buildCurrentProgram = async () => {
  ...
  try {
    ...
    const completedRustBuild = /\bFinished release\b/.test(result.stderr);
    const failed =
      !completedRustBuild &&
      /(^|\n)\s*(error|IDL error)(?:\[[^\]]+\])?:/i.test(result.stderr);
    const buildUpdate = {
      idl: result.idl,
      uuid: result.uuid ?? result.programInfo.uuid,
      lastBuildFailed: failed,
    };
    await PgProgramInfo.updateWorkspace(
      result.workspaceName,
      result.programInfo,
      buildUpdate
    );
    PgTerminal.println(
      improveOutput(result.stderr, result.workspaceName, buildUpdate.uuid)
    );
    return {
      failed,
      workspaceName: result.workspaceName,
      idl: result.idl,
      uuid: buildUpdate.uuid,
...
const buildProgram = async () => {
  ...
  if (!workspaceName) throw new Error("Open a workspace before building.");

  const buildFiles = getBuildFiles();
  const programInfo = PgProgramInfo.capture();
  const flags = { ...PgSettings.build.flags };
  const pythonFiles = buildFiles.filter(
    ([path]) => PgLanguage.getFromPath(path)?.name === "Python"
  );
  const result =
    pythonFiles.length > 0
      ? await buildPython(pythonFiles, programInfo.uuid, flags)
      : await buildRust(buildFiles, programInfo.uuid, flags);
  return { ...result, workspaceName, programInfo };
};
...
const buildPython = async (
  pythonFiles: TupleFiles,
  uuid: string | null,
  flags: typeof PgSettings.build.flags
) => {
  ...
  return await buildRust(rustFiles, uuid, flags);
};
...
const improveOutput = (
  ...
  output = output
    ...
    .replaceAll("solpg", workspaceName)
