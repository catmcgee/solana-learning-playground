...
export class PgTerminal {
  ...
  static async println(msg: any, opts?: PrintOptions) {
    PgCommon.createAndDispatchCustomEvent(
      PgTerminal.events.PRINT,
      String(msg ?? "")
    );
    await PgTerminal.run({ println: [msg, opts] });
  }

  /** Subscribe to the same output written to the Playground terminal. */
  static onDidPrint(cb: (message: string) => unknown) {
    return PgCommon.onDidChange(PgTerminal.events.PRINT, cb);
  }
