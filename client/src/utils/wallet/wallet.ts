...
import { SURFPOOL_RPC_URL } from "../../constants/connection";
...
class _PgWallet {
  ...
  static create(account: WalletAccount): Wallet {
    ...
    const assertSurfpoolScope = () => {
      let matchesSurfpoolSecret = false;
      try {
        const saved = JSON.parse(
          localStorage.getItem("solpg-learning-surfpool-keypair") ?? "null"
        ) as number[] | null;
        matchesSurfpoolSecret =
          saved?.length === account.kp.length &&
          saved.every((byte, index) => byte === account.kp[index]);
      } catch {
        matchesSurfpoolSecret = false;
      }

      const endpoint = PgConnection.current.rpcEndpoint;
      const isIsolatedLearningNetwork =
      ...
      if (
        (account.name === "Surfpool lab" ||
          account.name === "Surfpool wallet" ||
          matchesSurfpoolSecret) &&
        !isIsolatedLearningNetwork
      ) {
        throw new Error(
          "The Surfpool wallet can only sign on an isolated learning network."
        );
      }
    };
